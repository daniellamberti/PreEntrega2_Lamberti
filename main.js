...
function promedioExamenes(materia, cantExamenes,asignatura) {
	...
	while (i <= (cantExamenes-1)) {
		const notaExamen = parseFloat(prompt(`Ingresa la nota de tu examen ${i + 1} de ${asignatura}`));
	...
	return promedio;
             ...
             /* Función para obtener la nota máxima de los exámenes de cada asignatura. */
...
function notaMaxExam(materias){
	...
	for(let i = 0; i < materias.length; i++) {
		...
		return notaMasAlta;
            ...
            /* Función para obtener la nota mínima de los exámenes de cada asignatura. */
...
function notaMinExam(materias){
	...
	return notaMasBaja;
                    ...
                    /* Función para verificar si se aprobó cada asignatura */

function materiaAprob_noAprob(promedio, cantExam,asignatura) {
	alert(`El promedio de tus ${cantExam} notas de exámenes de ${asignatura} es ${promedio.toFixed(2)}`);
...
materiaAprob_noAprob(promedioInst,3,"instrumento");
...
materiaAprob_noAprob(promedioTeor,5,"teoria");
 ...
 materiaAprob_noAprob(promedioArm,4,"armonia");
...
materiaAprob_noAprob(promedioEns,3,"ensamble");
...
alert(
`Según los datos ingresados, considerando la nota promedio de las notas de ` +
`Instrumento, Teoría, Armonía y Ensamble, ` +
`más el agregado eventual de 1 punto por haber asistido a clases extras, ` +
`más el descuento eventual de 2 puntos por faltas,` +
`tu nota promedio final del curso es ${alumno.notaFinal()}`
);
